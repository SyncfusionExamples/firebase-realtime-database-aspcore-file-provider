...
{
    public class FirebaseFileProvider : FirebaseFileProviderBase
    {
        ...
        public FileManagerResponse Create(string path, string name, params FileManagerDirectoryContent[] data)
        {
            ...
            try
            {
                var idValue = this.firebaseGetData.Select(x => x.Id).ToArray().Select(int.Parse).ToArray().Max();
        ...
        public long GetItemSize(FileManagerDirectoryContent[] data)
        {
            foreach (var item in data)
            {
                ...
                var fileData = this.firebaseGetData.Where(x => x.ParentId == item.Id).Select(x => x.Size).ToArray();
                this.fileData = this.fileData + fileData.Sum();
                var dataFile = this.firebaseGetData.Where(x => x.ParentId == item.Id && x.IsFile == false).Select(x => x).ToArray();
        ...
        {
            var childs = this.firebaseGetData.Where(x => x.ParentId == item).Select(x => x).ToArray();
        ...
        public virtual FileManagerResponse Delete(string path, string[] names, params FileManagerDirectoryContent[] data)
        {
            ...
            {
                this.UpdateFirebaseJSONData();
                var emptyFolder = this.firebaseGetData.Where(x => x.ParentId == data[0].ParentId).Select(x => x).ToArray();
        ...
        public void copyFolderItems(FileManagerDirectoryContent item, FileManagerDirectoryContent target)
        {
            if (!item.IsFile)
            {
                var idVal = this.updatedNodeId();
                var i = this.firebaseGetData.Where(x => x.Id == item.Id).Select(x => x).ToList();
                FileManagerDirectoryContent CreateData = new FileManagerDirectoryContent()
                {
                    Id = (idVal + 1).ToString(),
                    Name = item.Name,
            ...
            var childs = this.firebaseGetData.Where(x => x.ParentId == item.Id).Select(x => x).ToArray();
            var idValue = this.updatedNodeId();
            if (childs.Length > 0)
            {
                foreach (FileManagerDirectoryContent child in childs)
                {
        ...
        public FileManagerResponse Copy(string path, string targetPath, string[] names, string[] renameFiles, FileManagerDirectoryContent targetData, params FileManagerDirectoryContent[] data)
        {
            ...
            var children = this.firebaseGetData.Where(x => x.ParentId == data[0].Id).Select(x => x.Id).ToList();
        ...
        public FileManagerResponse Move(string path, string targetPath, string[] names, string[] renameFiles, FileManagerDirectoryContent targetData, params FileManagerDirectoryContent[] data)
        {
            ...
            {
                var children = this.firebaseGetData.Where(x => x.ParentId == i.Id).Select(x => x.Id).ToList();
                if (children.IndexOf(targetData.Id) != -1 || data[0].Id == targetData.Id)
                {
                    ErrorDetails er = new ErrorDetails();
                    er.Code = "400";
                ...
                foreach (var item in data)
                {
                    try
                    {
                        ...
                        var targetItem = this.firebaseGetData.Where(x => x.Id == targetData.Id).Select(x => x).ToArray();
        ...
        public void updateChildPath(FileManagerDirectoryContent item)
        {
            ...
            if (childs.Length > 0)
            {
                foreach (FileManagerDirectoryContent child in childs)
                {
                    ...
                    var subchilds = this.firebaseGetData.Where(x => x.ParentId == child.Id).Select(x => x).ToArray();
        ...
        public FileManagerResponse Search(string path, string searchString, bool showHiddenItems = false, bool caseSensitive = false, params FileManagerDirectoryContent[] data)
        {
            ...
            {
                var i = new Char[] { '*' };
                var s = this.firebaseGetData.Where(x => x.Name.ToLower().Contains(searchString.TrimStart(i).TrimEnd(i).ToLower())).Select(x => x).ToArray();
        ...
        {
            var res = this.getFirebaseRootNode.Get(this.APIURL + "/" + this.rootNode + "/" + id);
            dynamic obj = Newtonsoft.Json.JsonConvert.DeserializeObject(res.JSONContent);
            var firebaseGetData = JsonConvert.DeserializeObject<FileManagerDirectoryContent[]>(getResponse.JSONContent);
            firebaseGetData = firebaseGetData.Where(c => c != null).ToArray();
            var cont = firebaseGetData.Where(x => x.Id == id).ToList();
        ...
        public virtual FileStreamResult Download(string path, string[] names, params FileManagerDirectoryContent[] data)
        {
            ...
            foreach (FileManagerDirectoryContent item in data)
            {
                var fileProperties = firebaseGetData.Where(x => x.Id == item.Id).Select(x => x.Content);
            ...
            else
            {
                ...
                ZipArchive archive;
                var tempPath = Path.Combine(Path.GetTempPath(), "files.zip");
        ...
        public void DownloadFolderFiles(string Id, string Name, ZipArchive archive, ZipArchiveEntry zipEntry)
        {
            ...
            var childFiles = this.firebaseGetData.Where(x => x.ParentId == Id).Select(x => x).ToArray();
            foreach (FileManagerDirectoryContent child in childFiles)
            {
                if (child.IsFile)
                {
                    var fileData = this.firebaseGetData.Where(x => x.ParentId == Id && x.IsFile == true).Select(x => x.Content).ToArray();
                    var fileContent = fileData.SelectMany(i => i).ToArray();
                    Stream file;
                ...
                {
                    this.localPath = Path.Combine(Path.GetTempPath() + Name);
                    Directory.CreateDirectory(this.localPath + "\\" + child.Name);
